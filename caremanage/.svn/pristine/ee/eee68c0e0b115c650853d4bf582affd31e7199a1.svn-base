	...
	<body>
        ...
        <div class="useractbox bodyline pad10" style="width:75%;float:right;" height=800px;>
			...
			<input type="hidden" name="userId" value="${userId}" />
					<label>日期：</label><input class="text150 timedate" type="text" name="start" value="$!date.format('yyyy-MM-dd',$!dateCond.start)"/> <label>至</label> <input class="text150 timedate" type="text" name="end" value="$!date.format('yyyy-MM-dd',$!dateCond.end)"/>
					<input class="button_add" type="submit" value="搜索">
					#if($sessionUser.cateEnum.type==4)
					   <li><input class="ibtn button_btn" type="button" onclick="javascripts:location.href='/professional/state/add?userId=${userId}'" value="新增"/></li>
					#end
