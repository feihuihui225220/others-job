	 ...
	 <dl style="margin-left: 5%;text-algin:center; ">
      	#foreach($a in $listAlbum)
       		<dd class="dd" style="float:left; margin-left:5%; ">
     	 		<img alt="$!a.albumName" id="$!a.id" style="width:180px;height:138px;" src="/$a.albumUrl"/>
         		...
         		<button style=" position: relative; top:-65px;" data-toggle="modal" data-target="#updatephoto" class="update button_gm"  >修改</button>  
 	   		</dd>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
				  <form role="form" id="addAlbumform" method="post" enctype="multipart/form-data" action="$request.contextPath/professional/album/add">
					<div class="form-group">
						 <label>活动名称</label><br/>
									    <input class="text150 timedate" id="start" type="text"
				                                   value="$!date.format('yyyy-MM-dd',$!dateCond.start)"> <text>-</text>
				                                    <input class="text150 timedate" id="jieshu" type="text"
				                                   value="$!date.format('yyyy-MM-dd',$!dateCond.end)">&nbsp;&nbsp;&nbsp;<input id="program" type="button"  class="button_add" value="搜索"/>&nbsp;&nbsp;&nbsp;
		...
		$.ajax({  
                ...
                success:function(list){
                	  ...
                	  for(var i=0;i<list.nActivityPlan.length;i++){
                	  	...
                	  	var option=$("<option class='option1' value='"+list.nActivityPlan[i].id+"'>"+list.nActivityPlan[i].planName+"</option>")
