	...
	funUploadFile : function(file){
	    ...
	    xhr.addEventListener("load", function(e){
	    	...
	    	if(self.uploadFile.length==0){
	    		...
	    		if(window.confirm("照片已经全部上传是否继续上传！")){
	    			 refresh();
    			}else{
    				location.href="/professional/album/findOneAlbum?id="+$('#album').val();
    			}
	    	}
	    }, false);  
	    // 错误
	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					url              : "/professional/photo/add?albumId="+$('#album').val(),  	      // 上传文件的路径
					fileType         : [],                                // 上传文件的类型
...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '							<img class="upload_image" src="$request.contextPath/assets/css/img/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>'; 
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				var type = newStr.split(".")[0].split("").reverse().join("");
				// 处理不同类型文件代表的图标
				var fileImgSrc = "$request.contextPath/assets/css/img/fileType/";
				if(type == "rar"){
