...
<div class="wrapper">
    ...
    <div class="body bodyline pad10">
        <div class="usertitle strong clearfix">
	        #set ($sessionUser=$!{session.getAttribute("sessionUser")})
					#if($!{sessionUser.getCateEnum()}!="STREET")  
        	<div class="f_r fr_itbn">状态选择：
                        <select id="status">
                           <option value="1">办理</option>
                           <option value="2">材料不全</option>
                           <option value="3">不办理</option>
                        </select>
                        <button class="addContact button_btn" id="saveBT">保存</button>
                        <button class="addContact button_btn" onclick="javascripts:history.go(-1)">返回</button>
          </div>
          #end
          	办理公共关系
          </div>
