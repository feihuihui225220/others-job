...
import com.anticw.aged.bean.gds.Bp;
import com.anticw.aged.bean.gds.BpInstantReport;
import com.anticw.aged.bean.gds.Case;
import com.anticw.aged.bean.gds.Case.Measure;
import com.anticw.aged.bean.gds.Patient;
import com.anticw.aged.bean.gds.Risktenyears;
...
public class Method {
	...
	// =====================================================================================
	// 获得WORD版分析报告
	public void getReportOfWord(String type,String patientId,String orgid,String auth,URI uri,Long id) {
		WebResource resource = Client.create().resource(uri);
		MultivaluedMap<String, String> params = new MultivaluedMapImpl();
		...
		if (status == 200) { // 如果执行成功
			...
			try {
				out = new FileOutputStream("D:\\report"+id+".docx");
				out.write(bytes);
				out.flush();
				out.close();
	...
	public boolean createOrupdatePatient(Patient patient,String patientId,String orgid,String auth,URI uri) {
		WebResource resource = Client.create().resource(uri);
	...
	// =====================================================================================
	// 获得患者当前的病情汇总
	public String getSummary(String patientId,String orgid,String auth,URI uri) {
		WebResource resource = Client.create().resource(uri);
		ClientResponse response = resource.path("patients").path(patientId)
	...
	// =====================================================================================
	public String getKeypoint(String patientId,String orgid,String auth,URI uri) {
		WebResource resource = Client.create().resource(uri);
		ClientResponse response = resource.path("patients").path(patientId)
